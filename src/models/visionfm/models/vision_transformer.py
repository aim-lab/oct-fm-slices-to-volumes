...
from src.models.visionfm.utils import trunc_normal_, load_pretrained_weights
from src.models.visionfm import ClsHead
...
class VisionFM(nn.Module):

    def __init__(self, finetune, **kwargs):
        super().__init__()

        self.backbone = VisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
        qkv_bias=True, num_classes=0)
        load_pretrained_weights(self.backbone, finetune, "teacher", None, None)


        self.n_intermediate_layers = 4
        self.classifier = ClsHead(embed_dim=768*self.n_intermediate_layers, num_classes=kwargs["num_classes"], layers=3)

    
    def forward(self, pixel_values):

        intermediate_output = self.backbone.get_intermediate_layers(pixel_values, self.n_intermediate_layers)
        embeddings = [x[:, 0] for x in intermediate_output] # only cls tokens
